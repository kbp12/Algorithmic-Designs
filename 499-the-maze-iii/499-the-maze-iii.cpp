...
public:
    ...
    void dfs(int i,int j,int a,int b, vector<vector<int>>& maze, vector<vector<int>>& dist, int len,string str){
        ...
        if(l!=j) dfs(i,l,a,b,maze,dist,len+abs(l-j),str+"l");
        while(u>0 and maze[u-1][j]!=1){ 
            u--;
            if(u==a and j==b)
                break;
        }
        if(u!=i) dfs(u,j,a,b,maze,dist,len+abs(i-u),str+"u");
        ...
        if(j!=r) dfs(i,r,a,b,maze,dist,len+abs(j-r),str+"r");
        while(d<maze.size()-1 and maze[d+1][j]!=1){ 
            d++;
            if(d==a and j==b)
                break;
        }
        if(d!=i) dfs(d,j,a,b,maze,dist,len+abs(i-d),str+"d");
        return;
    }
    string findShortestWay(vector<vector<int>>& maze, vector<int>& ball, vector<int>& hole) {
        ...
        dfs(ball[0],ball[1],hole[0],hole[1],maze,dist,0,"");
        //cout<<dist[hole[0]][hole[1]]<<endl;
